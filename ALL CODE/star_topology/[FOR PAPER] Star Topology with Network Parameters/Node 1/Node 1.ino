...
const char* serverIP = "10.158.66.31";
...
int numberOfTestingHours = 72; 
int currentMeasurement = 0;  // Measures the number of times sensor measurement has been done
unsigned long totalRuntime = numberOfTestingHours * 60 * 60 * 1000;  // Total test duration in milliseconds 
...
const int packetSize = 24; // each packet has 24 bytes because 4 bytes each air quality parameter (temp, humi, CO, pm2.5, pm10) and 4 bytes for node number 
const int numPackets = 12;  // Total number of packets to send, 12 per hour because 5 minutes each sensor measurement
const int totalSize = packetSize * numPackets * numberOfTestingHours;  // Total data size in bytes 
...
byte packet[packetSize]; // array to store the packets
...
void loop_DHT22_MQ135(){
  ...
  if (isnan(tempC) || isnan(humi)) { //don't send to thingsboard for temp and humi if no value
    numFailures++;
...
void loop() {
  ...
  if (packetCount == numPackets) {
    ...
    float throughput = (totalSize * 8) / (totalPacketLatency / 1000.0) / 1000.0;  // kbps
  ...
  // SENSOR MEASUREMENT AND FAILURE COUNTER
  currentMeasurement++;
