...
void loop_DHT22_MQ135(){
  ...
  if (isnan(tempC) || isnan(humi) || tempC == 0.00 || humi == 0.00) { //don't send to thingsboard for temp and humi if 0 or no value
    ...
    tb.sendTelemetryFloat("N2 Temperature", tempC);
    tb.sendTelemetryFloat("N2 Humidity", humi);
    bool dataSent_DHT22 = tb.sendTelemetryFloat("N2 Temperature", tempC) && tb.sendTelemetryFloat("N2 Humidity", humi);
  ...
  if (isnan(correctedPPM_ugm3) || correctedPPM_ugm3 == 0.00) { //don't send to thingsboard for CO if 0 or no value
    numFailures++;
    tb.sendTelemetryFloat("N2 Failures", numFailures);
  }
  else {
    tb.sendTelemetryFloat("N2 CO", correctedPPM_ugm3);
    bool dataSent_MQ135 = tb.sendTelemetryFloat("N2 CO", correctedPPM_ugm3);
    // Check if data was sent successfully
    if (dataSent_MQ135) { 
...
void loop_SDS011() {
  ...
  if (pm.isOk()) {
    saveReadings_SDS011(float(pm.pm25), float(pm.pm10));
     if (isnan(pm.pm25) || isnan(pm.pm10) || pm.pm25 == 0.00 || pm.pm10 == 0.00) { //don't send to thingsboard for PM2.5 and 10 if 0 or no value
       ...
       tb.sendTelemetryFloat("N2 PM2.5", pm.pm25);
       tb.sendTelemetryFloat("N2 PM10", pm.pm10);
       bool dataSent_NOVA = tb.sendTelemetryFloat("N2 PM2.5", pm.pm25) && tb.sendTelemetryFloat("N2 PM10", pm.pm10);
       // Check if data was sent successfully
       if (dataSent_NOVA) { 
  ...
  tb.sendTelemetryFloat("N2 Failures", numFailures); // sending the total numFailures currently for the loop
}
...
void loop() {
  ...
  if (currentTime - startTime >= totalRuntime) {
    ...
    tb.sendTelemetryFloat("N2 MTBF", mtbf);
    if (mtbf == 0) {
      Serial.print("\nMTBF: ");
